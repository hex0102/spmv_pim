...
/* Declaration of values */
    ...
    float  *values_A = NULL, *values_C = NULL;
    MKL_INT *columns_A = NULL, *columns_C = NULL;
    MKL_INT *rowIndex_A = NULL, *pointerB_C = NULL, *pointerE_C = NULL;

    // double  *rslt_mv = NULL, *rslt_mv_trans = NULL, *x = NULL, *y = NULL;

    // float   left, right, residual;
    ...
    sparse_matrix_t        csrA = NULL, csrC = NULL;
...
/* Printing usable data */
    // printf( "\n\n_______________Example program for MKL_SPARSE_s_SPMM_________________\n\n" );
    ...
    //     {
    //         printf(" (%5.0f, %6lld)", values_A[ii], columns_A[ii] ); fflush(0);
    //         ii++;
    //     }
    //     printf( "\n" );
    // }
...
/* Compute C = A * B  */
    CALL_AND_CHECK_STATUS(mkl_sparse_spmm( SPARSE_OPERATION_NON_TRANSPOSE, csrA, csrA, &csrC ),
    ...
    for (int k = 0; k < WARM_COUNT; k++) {
        mkl_sparse_spmm( SPARSE_OPERATION_NON_TRANSPOSE, csrA, csrA, &csrC );
    }
    ...
    printf (" == Sparse matrix-matrix multiplication using Intel(R) MKL dgemm completed == \n" " == at %.10f seconds == \n", (s_elapsed));
    printf ("%.10f\n", (s_elapsed));
...
/* Printing OUTPUT DATA */
    #ifdef PRINT_RESULT
      CALL_AND_CHECK_STATUS(mkl_sparse_s_export_csr( csrC, &indexing, &rows, &cols, &pointerB_C, &pointerE_C, &columns_C, &values_C ),
      ...
      for( i = 0; i < M; i++ )
      {
          ...
          {
              printf(" (%5.0f, %6lld)", values_C[ii], columns_C[ii] ); fflush(0);
              ii++;
          }
          printf( "\n" );
      }
